 ...
 * 
 * 核心设计：
 * 1. 虚拟化显示：根据位置计算Address
 * 2. 页面对齐：native 读取时Address必须对齐To PAGE_SIZE
 * 3. 缓存机制：缓存已读取的页面数据（最多4页）
 * 4. 边界扩展：滚动To边界时自动扩展AddressRange（仅无限滚动模式）
 * 5. 预加载：跳转时预加载前、中、后三页
 * 6. 固定页面模式：只显示一页Memory，禁用边界扩展
 */
class InfiniteMemoryAdapter(
    ...
    /**
     * 安全地通知页面数据变化
     * 如果 RecyclerView 正在布局或滚动，则延迟To下一帧执行
     */
    ...
    override fun getItemViewType(position: Int): Int {
        if (!infiniteScrollEnabled) {
            when (position) {
                0 -> return VIEW_TYPE_NAVIGATION  // Previous page
                itemCount - 1 -> return VIEW_TYPE_NAVIGATION  // Next page
            }
        }
    ...
    override fun getItemId(position: Int): Long {
        if (!infiniteScrollEnabled) {
            when (position) {
                0 -> return Long.MIN_VALUE  // Previous page导航
                itemCount - 1 -> return Long.MAX_VALUE  // Next page导航
            }
            // Memory行位置需要偏移1（因为位置0是Previous page导航）
    ...
    inner class NavigationViewHolder(
        ...
        init {
            binding.root.setOnClickListener {
                val targetAddress = if (isNextPage) {
                    // Next page：当前基Address + 一页大小
                    baseAddress + PAGE_SIZE
                } else {
                    // Previous page：当前基Address - 一页大小（最小为0）
        ...
        fun bind(isNext: Boolean) {
            ...
            if (isNext) {
                binding.navigationText.text = "Next page → $formattedAddress"
            } else {
                binding.navigationText.text = "← Previous page $formattedAddress"
            }
        }
    }
