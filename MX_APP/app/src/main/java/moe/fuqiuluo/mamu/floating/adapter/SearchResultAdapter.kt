...
class SearchResultAdapter(
    ...
    // 使用标志位 + 例外集合，避免Select All/Invert Selection时 O(n) 的集合操作
    // isAllSelected=true 时: 所有位置默认选中，deselectedPositions 存储Cancel选择的位置
    ...
    // MemoryRange列表 (保存主要是为了显示MemoryRange简称和颜色)
    private var ranges: List<DisplayMemRegionEntry>? = null
    // 预排序的Range列表，用于二分查找 (按 start Address排序)
    ...
    private fun toggleSelection(position: Int, selected: Boolean) {
        if (isAllSelected) {
            // Select All模式下，操作 deselectedPositions
            if (selected) {
            ...
            // 非Select All模式下，操作 selectedPositions
            if (selected) {
    ...
    /**
     * 根据Address更新单个Search结果项的值
     * @param address 要更新的Address
    ...
    /**
     * 获取所有选中项的原生Address数组
     * @return 选中项的原生Address数组
     */
    fun getNativePositions(): LongArray {
        return if (isAllSelected) {
            // Select All模式：返回所有未Cancel选择的位置
            results.indices
    ...
    /**
     * 设置MemoryRange列表
     * @param newRanges 新的MemoryRange列表
     */
    ...
    /**
     * 使用二分查找定位Address所属的MemoryRange
     * 时间复杂度: O(log n)，相比之前的 O(n) 线性查找大幅提升
     *
     * @param address 要查找的Address
     * @return 对应的 MemoryRange，找不To返回 MemoryRange.O
     */
    private fun findMemoryRangeByAddress(address: Long): MemoryRange {
        ...
        // 二分查找：找To最后一个 start <= address 的Range
        var low = 0
    ...
    /**
     * Invert Selection - O(1) 操作，只切换标志位并交换集合
     */
    fun invertSelection() {
        // O(1) 操作：切换Select All标志位，交换两个集合的角色
    ...
    companion object {
         ...
         * 对于 Pattern 类型，直接显示 Rust 返回的Hexadecimal内容
         */
            ...
            return try {
                when (type) {
                    ...
                    DisplayValueType.PATTERN -> {
                        // Pattern 类型直接显示 Rust 返回的Hexadecimal内容
                        value
                    }
                    else -> value
                }
    ...
    inner class ViewHolder(
        ...
        fun bind(item: SearchResultItem, position: Int) {
            when (item) {
                is ExactSearchResultItem -> {
                    binding.apply {
                        ...
                        // MemoryRange简称和颜色 - 使用二分查找 O(log n) 替代线性查找 O(n)
            ...
            // Checkbox选中状态 - 使用 isPositionSelected() 支持Select All标志位
