...
fun TutorialPointerChainScreen(
    ...
    data class PointerChain(
        ...
        val targetAddress: ULong,    // Target address
        val targetValue: Int,        // 目标值
        val baseOffset: Long,        // 基址To第一层的偏移
        val offset1: Long,           // 第一层To第二层的偏移
        val offset2: Long            // 第二层To目标的偏移
    )
    ...
    DisposableEffect(Unit) {
        ...
        if (targetAddr != 0UL && level2Addr != 0UL && level1Addr != 0UL && staticBase != 0UL) {
            ...
            // 构建链：staticBase 存储 level1 的Address
            // staticBase 是静态变量本身的Address（在 .data/.bss 段）
    ...
    // 定期FromMemory读取值（检测用户是否通过悬浮窗修改了值或找To了指针链）
    LaunchedEffect(pointerChain) {
        pointerChain?.let { chain ->
            while (true) {
                ...
                // 检查用户是否通过指针扫描找To了正确的链
                // 这里需要实现验证逻辑
                // todo：如果用户找To了链并且能看To结果，就算成功
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Pointer-chain Search Practice") },
                navigationIcon = {
            ...
            Column(
                ...
                Card(
                    ...
                    Column(
                        ...
                        Text(
                            text = "Use the pointer scan feature to find a pointer chain to the target address below. Pointer chains can stay valid after app restarts and are a core memory editing technique.",
                ...
                Card(
                    ...
                    Column(
                        ...
                        Text(
                            text = "A pointer chain is a multi-level pointer structure, for example:",
                        ...
                        Spacer(modifier = Modifier.height(Dimens.spacingXs(adaptiveLayout)))
                        Text(
                            text = "It starts from a static region (.data/.bss) and follows several pointer hops to the final value. Because the static region address stays stable across restarts, the chain can remain valid.",
                            style = MaterialTheme.typography.bodySmall,
                            color = MaterialTheme.colorScheme.onTertiaryContainer
                        )
                    }
                ...
                Card(
                    ...
                    Column(
                        ...
                        val steps = listOf(
                            "Launch the floating window and bind the process: moe.fuqiuluo.mamu",
                            "Find the target address in the search results",
                            "Long-press the target address and choose Pointer Scan",
                            "Wait for the scan to finish (this may take a few minutes)",
                            "Review the results and identify a pointer chain",
                            "Verify that the pointer chain is correct"
                        )
                            ...
                            Card(
                                ...
                                Column(
                                    ...
                                    Row(
                                        ...
                                        Text(
                                            text = "Reference chain structure",
                    ...
                    Card(
                        ...
                        Row(
                            ...
                            Column {
                                ...
                                Text(
                                    text = "You have learned the basics of pointer-chain searching.",
                                    style = MaterialTheme.typography.bodySmall,
                                    color = MaterialTheme.colorScheme.onPrimaryContainer
                                )
                            }
                ...
                // 提示信息
                Text(
                    text = "Tip: pointer scans can take a while, so please be patient.",
                    style = MaterialTheme.typography.bodySmall,
                    color = MaterialTheme.colorScheme.onSurfaceVariant,
