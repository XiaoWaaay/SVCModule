...
class ImportAddressDialog(
        ...
        coroutineScope.launch {
            ...
            if (result != null) {
                ...
                if (pid > 0 && currentPid > 0 && pid != currentPid) {
                    notification.showWarning("Warning: the PID in the file ($pid) does not match the current process PID ($currentPid)")
                }

                if (addresses.isNotEmpty()) {
                    notification.showSuccess("Successfully imported ${addresses.size} addresses")
                    onImportComplete?.invoke(addresses)
                } else {
                    notification.showError("Import failed: the file does not contain a valid address")
                }
            } else {
    ...
    /**
     * 解析单行Address数据
     * 格式：名称|Address(小写hex)|类型|值|冻结|0|0|0|Permissions|区域名称|偏移
     */
