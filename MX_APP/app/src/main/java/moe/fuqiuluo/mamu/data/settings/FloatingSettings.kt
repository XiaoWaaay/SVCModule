...
private const val DEFAULT_RUDE_SEARCH = true // 默认不启用粗鲁Search模式
...
private const val DEFAULT_KEYBOARD_STATE = 1 // 默认Expand (0=Collapse, 1=Expand, 2=功能)
...
var MMKV.memoryAccessMode: Int
    ...
    set(value) {
        WuwaDriver.setMemoryAccessMode(value) // 每次改变都同步To底层驱动
...
/**
 * 兼容模式
 * true = 启用兼容模式，所有Search结果以模糊Search格式存储
 * false = 标准模式，精确Search和模糊Search结果分别存储
 */
 ...
 * false = 固定页面模式，只显示一页Memory（base address To base address + PAGE_SIZE）
 */
