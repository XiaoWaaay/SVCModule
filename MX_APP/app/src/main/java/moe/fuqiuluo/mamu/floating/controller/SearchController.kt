...
class SearchController(
    ...
    /**
     * 订阅Address值变更事件
     * 当保存Address界面修改值时，同步更新Search结果界面的显示
     */
    ...
    /**
     * 订阅Search结果更新事件（From保存Address界面Search后）
     */
    ...
    private fun setupRecyclerView() {
            ...
            onItemDelete = { item ->
                ...
                coroutineScope.launch {
                    ...
                    if (success) {
                        ...
                        notification.showError("Delete failed")
                    }
                }
            }
        )
        ...
        binding.resultsRecyclerView.apply {
            ...
            setItemViewCacheSize(20) // 默认是2，提高To20可以减少rebind

            // 性能优化：设置固定大小，避免每次数据变化都重新测量
            setHasFixedSize(true)

            // 性能优化：禁用变化动画，避免Select All/Invert Selection时的闪烁和卡顿
            ...
            runCatching {
                ...
                if (addresses.isEmpty()) {
                    if (offset == 0) {
                        searchResultAdapter.clearResults()
                        showEmptyState(true)
                        notification.showError("No results found ($limit)")
                    }
                    return@launch
                }
                ...
                notification.showError("Failed to load results: ${e.message}")
            }
        }
    }
    ...
    fun onSearchCompleted(ranges: List<DisplayMemRegionEntry>, totalFound: Long) {
        ...
        if (totalCount > 0) {
            val itemCountPerPage = mmkv.searchPageSize
            val limit = itemCountPerPage.coerceAtMost(totalCount)
            updateSearchResultCount(itemCountPerPage, totalCount) // SearchCompleted
    ...
    private fun showSearchDialog(clipboardManager: ClipboardManager) {
        if (!WuwaDriver.isProcessBound) {
            notification.showError("Please select a process first")
            return
        }
    ...
    /**
     * 隐藏Search进度对话框（如果正在Search）
     * 用于退出全屏时隐藏进度 UI，但后台Search继续
     */
    ...
    /**
     * 如果正在Search，重新显示Search进度对话框
     * 用于重新进入全屏时Restore进度 UI
     */
    ...
    /**
     * 如果模糊Search已Completed且有结果，重新显示模糊Search对话框
     * 用于重新进入全屏时Restore对话框，让用户继续细化
     */
    ...
    private fun applyFilter(state: FilterDialogState) {
        ...
        coroutineScope.launch {
            ...
            notification.showSuccess("Filter settings applied" + if (state.isFilterEnabled()) ": ${state.getFilterDescription()}" else "")
        }
    }
    ...
    private fun showRemoveDialog() {
        ...
        if (totalCount == 0) {
            notification.showWarning("There are no results to remove")
            return
        }
    ...
    private fun restoreAndRemoveSelected() {
        ...
        notification.showWarning("Restore-and-remove is still under development")
    }
    ...
    private fun restoreSelectedItems() {
        ...
        if (!WuwaDriver.isProcessBound) {
            notification.showError("No process is selected")
            return
        }
    ...
    private fun showRestoreResultNotification(
        ...
        when {
            totalSelected == noBackupCount -> {
                notification.showWarning("None of the selected items has a backup")
            }
            failureCount == 0 && noBackupCount == 0 -> {
                notification.showSuccess("Successfully restored $successCount addresses")
            }
            else -> {
                val message = buildString {
                    if (successCount > 0) append("Success: $successCount")
                    if (failureCount > 0) {
                    ...
                    if (noBackupCount > 0) {
                        if (isNotEmpty()) append(", ")
                        append("No backup: $noBackupCount")
                    }
                }
    ...
    private fun removeSelected() {
        ...
        coroutineScope.launch {
            ...
            if (success) {
                notification.showSuccess("Removed ${selectedItems.size} results")
                ...
                notification.showError("Bulk delete failed")
            }
        }
    }
    ...
    private fun setSelectedAsSearchResults() {
        val totalCount = SearchEngine.getTotalResultCount().toInt()
        if (totalCount == 0) {
            notification.showWarning("There are no actionable results")
            return
        }
        ...
        coroutineScope.launch {
            ...
            // 直接调用 keepOnlyResults，保留选中的项，Delete其他所有项
            ...
            if (success) {
                notification.showSuccess("Set ${selectedItems.size} selected items as search results")
    ...
    private fun showAddressActionDialog(result: SearchResultItem) {
        val address = when (result) {
            ...
            is FuzzySearchResultItem -> result.address
                ...
                notification.showError("Invalid search result type")
                return
            }
        }
        ...
        fun batchModifyValues(
            ...
            coroutineScope.launch {
                ...
                try {
                    ...
                    notification.showError(
                        "Batch update failed: ${e.message ?: "Unknown error"}"
                    )
                }
            }
        }
    ...
    private fun onPointerScanCompleted(ranges: List<DisplayMemRegionEntry>, chains: List<PointerChainResult>) {
        ...
        searchResultAdapter.setRanges(ranges) // 指针扫描结果不需要MemoryRange
