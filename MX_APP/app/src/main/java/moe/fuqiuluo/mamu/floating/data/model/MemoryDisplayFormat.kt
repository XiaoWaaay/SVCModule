...
enum class MemoryDisplayFormat(
    ...
    val appendCode: Boolean = false, // 附加codeTo结尾
) {
    HEX_BIG_ENDIAN(
        code = "h",
        displayName = "Reversed hex (big-endian)",
    ...
    ARM32(
        code = "A",
        displayName = "ARM 32 instruction",
    ...
    /**
     * 转换为对应的 DisplayValueType（用于保存Address）
    ...
    companion object {
        ...
        /**
         * From格式列表中筛选出可保存的类型（有对应 DisplayValueType 的格式）
         */
        ...
        /**
         * 根据选中的格式计算Hexadecimal显示的字节数（使用最大的字节数，排除Int.MAX_VALUE）
         */
