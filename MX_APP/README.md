...
For signed release builds, the Gradle script reads:

- `RELEASE_KEYSTORE_PATH`
- `RELEASE_KEYSTORE_PASSWORD`
- `RELEASE_KEY_ALIAS`
- `RELEASE_KEYSTORE_TYPE` (`JKS` or `PKCS12`, defaults to `JKS` for `.keystore` / `.jks` files)

If you only need an unsigned release artifact for testing, you can run:

```bash
./gradlew assembleRelease -PunsignedRelease=true
```

The GitHub Actions `Android Release Build` workflow also exposes manual `workflow_dispatch` checkboxes so you can independently build a signed release APK, an unsigned release APK, and/or a debug APK in the same run. Signed release builds are skipped automatically when the configured keystore is missing or invalid.
